...
func TestUpdatebookbyID(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			service: func() *mocks.Service {
				...
				mockService.On("UpdatebookbyID", mock.Anything).Return(nil)
				return mockService
			}(),
		...
		},
		{
			...
			service: func() *mocks.Service {
				mockService := new(mocks.Service)
				mockService.On("UpdatebookbyID", mock.Anything).Return(errors.New("insert error"))
				return mockService
			}(),
			requestBody: models.Book{
				Name: "BookName",
	...
	for _, tC := range testCases {
		t.Run(tC.desc, func(t *testing.T) {
			...
			server := gin.Default()
			server.POST("/api/books", handler.UpdateBookByID)
